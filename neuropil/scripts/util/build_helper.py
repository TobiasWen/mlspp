...
if __name__ == "__main__":
    ...
    parser.add_argument('--prepare_ci',help='Prepare CI file for dynamic runners',action="store_true")
    ...
    if args.update_strings:
        ...
        shutil.copy(os.path.join("README.md"),                  os.path.join('build', 'package', "README"))
        ...
        for include_file in glob.glob(os.path.join("include","neuropil*.h")):
            shutil.copy(include_file, os.path.join('build', 'package', "include", os.path.basename(include_file)))
        ...
        for (dirpath, dirnames, filenames) in os.walk(os.path.join('build', 'package', "bin")):
            for filename in filenames:
                sign_file(os.path.join(dirpath,filename), args.sign_file,  args.pw)
        ...
        if args.gitlab_pipeline_cleanup:
            ...
            for packages_info in packages_infos:
                if packages_info['version'] == semver_str:
                    ...
                    package_file_infos = requests.get(f"{CI_API_V4_URL}/projects/{CI_PROJECT_ID}/packages/{package_id}/package_files", headers=headers).json()
        ...
        if args.gitlab_release:
            ...
            project_config = requests.get(f"{CI_API_V4_URL}/projects/{CI_PROJECT_ID}", headers=headers).json()
            ...
            r = requests.post(f"{CI_API_V4_URL}/projects/{CI_PROJECT_ID}/releases", json=release_payload, headers=headers)
            try:
                r.raise_for_status()
            except:
        ...
        if args.gitlab_latest_release:
            ...
            r = requests.get(f"{CI_API_V4_URL}/projects/{CI_PROJECT_ID}/releases?order_by=released_at&&sort=desc", headers=headers)
            try:
                r.raise_for_status()
            except:
            ...
            r = requests.delete(f"{CI_API_V4_URL}/projects/{CI_PROJECT_ID}/protected_tags/latest_release", headers=headers)
            try:
                r.raise_for_status()
            except:
            ...
            r = requests.delete(f"{CI_API_V4_URL}/projects/{CI_PROJECT_ID}/repository/tags/latest_release", headers=headers)
            try:
                r.raise_for_status()
            except:
            ...
            r = requests.post(f"{CI_API_V4_URL}/projects/{CI_PROJECT_ID}/releases", json=copy_release_config, headers=headers)
            try:
                r.raise_for_status()
            except:
            ...
            r = requests.post(f"{CI_API_V4_URL}/projects/{CI_PROJECT_ID}/protected_tags?name=latest_release&create_access_level=40", headers=headers)
            try:
                r.raise_for_status()
            except:
        ...
        tags = [(f'neuropil-{t.strip()}',t.strip()) for t in DYNAMIC_BUILDERS.splitlines()]
        tags = list(set(tags))
        pprint(tags)
