...
np_msgproperty_t *_np_msgproperty_get(np_state_t *context, np_msg_mode_type mode_type, const char *subject) {
    log_trace_msg(LOG_TRACE,
                  "start: np_msgproperty_t* np_msgproperty_get(context, np_msg_mode_type mode_type, const char* subject){");
    assert(subject != NULL);
    ...
    if (FLAG_CMP(mode_type, INBOUND)) {   // search receiving property
        ...
        log_debug_msg(LOG_DEBUG, "get i: msgproperty %s: get %p from list: %p (%d)", subject,
                      sll_first(my_property_key_rx->entities)->val, my_property_key_rx,
                      sll_size(my_property_key_rx->entities));
    ...
    if (FLAG_CMP(mode_type, OUTBOUND)) {
        ...
        log_debug_msg(LOG_DEBUG, "get o: msgproperty %s: get %p from list: %p(%d)", subject,
                      sll_first(my_property_key_tx->entities)->val, my_property_key_tx,
                      sll_size(my_property_key_tx->entities));
...
bool _np_msgproperty_check_msg_uniquety_in(np_msgproperty_t *self, np_message_t *msg_to_check) {
    ...
    if (self->unique_uuids_check) {
        ...
        if (np_tree_find_str(self->unique_uuids_in, _to_check) == NULL) {
            np_tree_insert_str(self->unique_uuids_in, _to_check,
                               np_treeval_new_d(_np_message_get_expiery(msg_to_check)));
        } else {
            ret = false;
        }
    }
...
bool _np_msgproperty_check_msg_uniquety_out(np_msgproperty_t *self, np_message_t *msg_to_check) {
    ...
    if (self->unique_uuids_check) {
        ...
        if (np_tree_find_str(self->unique_uuids_out, _to_check) == NULL) {
            np_tree_insert_str(self->unique_uuids_out, _to_check,
                               np_treeval_new_d(_np_message_get_expiery(msg_to_check)));
        } else {
            ret = false;
        }
...
void _np_msgproperty_job_msg_uniquety(np_msgproperty_t *self) {
    ...
    if (self->unique_uuids_check) {
        ...
        if (iter_to_rm != NULL) {
            log_debug_msg(LOG_DEBUG | LOG_MSGPROPERTY,
                          "UNIQUITY removing %"PRIu32" from %"PRIu16" items from unique_uuids for %s",
...
void _np_msgproperty_cleanup_response_handler(np_msgproperty_t *self) {
    ...
    RB_FOREACH(iter_tree, np_tree_s, self->response_handler) {
        ...
        if (current->received_at < now && current->received_at > current->send_at) {   // notify about ack response
            response_event.type = (evt_internal | evt_response);
            sll_append(char_ptr, to_remove, iter_tree->key.value.s);
            handle_event = true;
        } else if (current->expires_at < now) {   // notify about timeout
        ...
        if (handle_event) {
            ...
            if (!_np_dhkey_equal(&current->msg_dhkey, &_null)) {  // clean up message redlivery
                response_event.target_dhkey = current->msg_dhkey;
                _np_keycache_handle_event(context, current->msg_dhkey, response_event, false);
            }
            if (!_np_dhkey_equal(&current->dest_dhkey, &_null)) {   // clean up ping ack
    ...
    if (iter_to_rm != NULL) {
        log_debug_msg(LOG_DEBUG | LOG_MSGPROPERTY,
                      "RESPONSE removing %"PRIu32" from %"PRIu16" items from response_handler",
...
void _np_msgproperty_check_sender_msgcache(np_msgproperty_t *send_prop) {
    ...
    while (0 < msg_available && msg_available <= send_prop->cache_size) {
        ...
        if (NULL != msg_out) {
            ...
            np_ref_obj(np_message_t, msg_out,
                       ref_message_in_send_system); // this ref reason has been removed on first try, re-add
            np_util_event_t send_event = {.type=(evt_internal | evt_userspace |
                                                 evt_message), .context=context, .user_data=msg_out, .target_dhkey=target_dhkey};
...
void _np_msgproperty_check_receiver_msgcache(np_msgproperty_t *recv_prop, np_dhkey_t from) {
    ...
    while (0 < msg_available && msg_available <= recv_prop->cache_size) {
        ...
        if (NULL != msg_in) {
            ...
            np_util_event_t msg_in_event = {.type=(evt_external |
                                                   evt_message), .context=context, .target_dhkey=in_handler, .user_data=msg_in};
...
void _np_msgproperty_add_msg_to_send_cache(np_msgproperty_t *msg_prop, np_message_t *msg_in) {
    ...
    if (msg_prop->cache_size <= sll_size(msg_prop->msg_cache_out)) {
        ...
        if (FLAG_CMP(msg_prop->cache_policy, OVERFLOW_PURGE)) {
            log_debug_msg(LOG_MSGPROPERTY | LOG_DEBUG, "OVERFLOW_PURGE: discarding message in send msgcache for %s",
...
void _np_msgproperty_cleanup_receiver_cache(np_msgproperty_t *msg_prop) {
    ...
    while (iter_prop_msg_cache_in != NULL) {
        ...
        if (_np_message_is_expired(old_msg)) {
            log_msg(LOG_WARN, "purging expired message (subj: %s, uuid: %s) from receiver cache ...",
                    msg_prop->msg_subject, old_msg->uuid);
...
void _np_msgproperty_cleanup_sender_cache(np_msgproperty_t *msg_prop) {
    ...
    while (iter_prop_msg_cache_out != NULL) {
        ...
        np_message_t *old_msg = old_iter->val;
        if (_np_message_is_expired(old_msg)) {
            log_msg(LOG_WARN, "purging expired message (subj: %s, uuid: %s) from sender cache ...",
                    msg_prop->msg_subject, old_msg->uuid);
...
void _np_msgproperty_add_msg_to_recv_cache(np_msgproperty_t *msg_prop, np_message_t *msg_in) {
    ...
    if (msg_prop->cache_size <= sll_size(msg_prop->msg_cache_in)) {
        ...
        if (FLAG_CMP(msg_prop->cache_policy, OVERFLOW_PURGE)) {
            log_debug_msg(LOG_MSGPROPERTY | LOG_DEBUG, "OVERFLOW_PURGE: discarding message in recv msgcache for %s",
                          msg_prop->msg_subject);
            np_message_t *old_msg = NULL;
...
void __np_msgproperty_redeliver_messages(np_msgproperty_t *self) {
    ...
    RB_FOREACH(iter_tree, np_tree_s, self->redelivery_messages) {
        ...
        if (current->redelivery_at + resend_interval < now) {   // send message redelivery attempt
            current->redelivery_at = now;
            np_ref_obj(np_message_t, current, ref_message_in_send_system);
            np_util_event_t message_event = {.type=(evt_message |
                                                    evt_internal), .context=context, .user_data=current, .target_dhkey=property_dhkey};
...
np_aaatoken_t *
_np_msgproperty_get_mxtoken(np_util_statemachine_t *statemachine, NP_UNUSED const np_util_event_t event) {
    np_ctx_memory(statemachine->_user_data);
    ...
    if (_np_dhkey_equal(&my_property_key->dhkey, &send_dhkey)) {
        log_debug_msg(LOG_DEBUG, "start: void _np_msgproperty_get_mxtoken SENDER(...){ %s",
                      _np_key_as_str(my_property_key));
        token_list = ledger->send_tokens;
    }

    if (_np_dhkey_equal(&my_property_key->dhkey, &recv_dhkey)) {
        log_debug_msg(LOG_DEBUG, "start: void _np_msgproperty_get_mxtoken RECEIVER(...){%s",
                      _np_key_as_str(my_property_key));
...
void _np_msgproperty_upsert_token(np_util_statemachine_t *statemachine, NP_UNUSED const np_util_event_t event) {
    ...
    np_pll_t(np_aaatoken_ptr, token_list = NULL);
    if (_np_dhkey_equal(&my_property_key->dhkey, &send_dhkey)) {
        log_debug_msg(LOG_MSGPROPERTY | LOG_DEBUG, "start: void _np_msgproperty_upsert_token SENDER(...){%s",
                      _np_key_as_str(my_property_key));
        token_list = ledger->send_tokens;
    } else if (_np_dhkey_equal(&my_property_key->dhkey, &recv_dhkey)) {
        log_debug_msg(LOG_MSGPROPERTY | LOG_DEBUG, "start: void _np_msgproperty_upsert_token RECEIVER(...){%s",
                      _np_key_as_str(my_property_key));
        token_list = ledger->recv_tokens;
    } else {
    ...
    do {
        if (NULL == iter) {
            log_debug_msg(LOG_MSGPROPERTY | LOG_DEBUG, "--- new mxtoken for subject: %25s --------",
                          property->msg_subject);
            ...
            ref_replace_reason(np_aaatoken_t, msg_token_new, "_np_token_factory_new_message_intent_token",
                               ref_aaatoken_local_mx_tokens);
        } else if ((iter->val->expires_at - now) <=
                   fmax(property->token_min_ttl + 1.0, MSGPROPERTY_DEFAULT_MIN_TTL_SEC)) {   // Create a new msg token
            log_debug_msg(LOG_MSGPROPERTY | LOG_DEBUG, "--- refresh mxtoken for subject: %25s --------",
                          property->msg_subject);
            np_aaatoken_t *msg_token_new = _np_token_factory_new_message_intent_token(property);
            ...
            log_debug(LOG_MSGPROPERTY, "--- update mxtoken for subject: %25s token: %s--------", property->msg_subject,
                      iter->val->uuid);
            ...
            r = np_set_data(iter->val->attributes,
                            (struct np_data_conf) {.key = "max_threshold", .type = NP_DATA_TYPE_UNSIGNED_INT},
                            max_threshold);
...
void np_msgproperty_from_user(np_state_t *context, np_msgproperty_t *dest, struct np_mx_properties *src) {
    ...
    if (src->reply_subject[0] != '\0' &&
        (dest->rep_subject == NULL || strncmp(dest->rep_subject, src->reply_subject, 255) != 0)) {
...
void __np_set_property(np_util_statemachine_t *statemachine, const np_util_event_t event) {
    ...
    log_debug_msg(LOG_DEBUG, "sto  :msgproperty %s: %p added to list: %p / %p", property->msg_subject, property,
                  my_property_key, my_property_key->entities);


    if (property->is_internal == false) {
        _np_msgproperty_create_token_ledger(statemachine, event);

        if (false == sll_contains(np_evt_callback_t, property->clb_outbound, _np_out_callback_wrapper,
                                  np_evt_callback_t_sll_compare_type)) {   // first encrypt the payload for receiver
            sll_append(np_evt_callback_t, property->clb_outbound, _np_out_callback_wrapper);
        }
        if (false == sll_contains(np_evt_callback_t, property->clb_outbound, _np_out_default,
                                  np_evt_callback_t_sll_compare_type)) {   // then route and send message
...
void __np_msgproperty_send_available_messages(np_util_statemachine_t *statemachine, const np_util_event_t event) {
    ...
    np_util_event_t available_event = {.type=(evt_internal |
                                              evt_message), .context=context, .target_dhkey=target_dhkey};

    double now = np_time_now();

    if (_np_dhkey_equal(&property_key->dhkey, &send_dhkey) &&
        (now - property->last_intent_tx_update) >
        property->token_min_ttl) {   // send our token, search for receiver of messages
        np_message_t *msg_out = NULL;
        np_new_obj(np_message_t, msg_out, ref_message_in_send_system);
        _np_message_create(msg_out, target_dhkey, context->my_node_key->dhkey, _NP_MSG_AVAILABLE_SENDER,
                           np_tree_clone(intent_data));

        log_msg(LOG_INFO,
                "sending available message for %s as a sender: _NP_MSG_AVAILABLE_SENDER {msg uuid: %s / intent uuid: %s)",
                property->msg_subject, msg_out->uuid, intent_token->uuid);
    ...
    if (_np_dhkey_equal(&property_key->dhkey, &recv_dhkey) &&
        ...
        (sll_contains(np_evt_callback_t, property->clb_inbound, _np_in_callback_wrapper,
                      np_evt_callback_t_sll_compare_type) ||
         sll_contains(np_evt_callback_t, property->clb_inbound, _np_in_mls_callback_wrapper,
                      np_evt_callback_t_sll_compare_type))) {   // send our token, search for sender of messages
        np_message_t *msg_out = NULL;
        np_new_obj(np_message_t, msg_out, ref_message_in_send_system);
        _np_message_create(msg_out, target_dhkey, context->my_node_key->dhkey, _NP_MSG_AVAILABLE_RECEIVER,
                           np_tree_clone(intent_data));
...
void
__np_msgproperty_send_pheromone_messages(np_util_statemachine_t *statemachine, NP_UNUSED const np_util_event_t event) {
    np_ctx_memory(statemachine->_user_data);
    ...
    np_util_event_t pheromone_event = {.type=(evt_internal |
                                              evt_message), .context=context, .target_dhkey=target_dhkey};
    ...
    if (is_send) {
        np_tree_insert_int(bloom_data, -(target_dhkey.t[0] % 257) - 1,
        ...
        if (FLAG_CMP(property->ack_mode, ACK_DESTINATION) ||
            ...
            np_tree_insert_int(bloom_data, (ack_dhkey.t[0] % 257) + 1,
    ...
    if (last_pheromone_update > 0 &&
        ...
        np_message_t *msg_out = NULL;
        np_new_obj(np_message_t, msg_out, ref_message_in_send_system);
        _np_message_create(msg_out, target_dhkey, context->my_node_key->dhkey, _NP_MSG_PHEROMONE_UPDATE,
                           np_tree_clone(bloom_data));
...
void __np_property_handle_out_msg(np_util_statemachine_t *statemachine, const np_util_event_t event) {
    ...
    if (property->is_internal == false) {
        ...
        while (iter_usercallbacks != NULL && ret) {
            ret &= iter_usercallbacks->val->fn(context, msg_out, (msg_out == NULL ? NULL : msg_out->body),
...
void __np_response_handler_set(np_util_statemachine_t *statemachine, const np_util_event_t event) {
    ...
    if (property->is_internal) {   // registration of response handler for message type NP_ACK
        ...
        CHECK_STR_FIELD_BOOL(property->redelivery_messages, responsehandler->uuid, msg_tree_elem, "NO UUID FOUND") {
            log_msg(LOG_INFO, "message %s / %s acknowledged or timed out", responsehandler->uuid,
                    property->msg_subject);
...
void __np_property_handle_intent(np_util_statemachine_t *statemachine, const np_util_event_t event) {
    ...
    if (_np_dhkey_equal(&target_inbound_dhkey, &my_property_key->dhkey) &&
        (sll_contains(np_evt_callback_t, real_prop->clb_inbound, _np_in_callback_wrapper,
                      np_evt_callback_t_sll_compare_type) ||
         sll_contains(np_evt_callback_t, real_prop->clb_inbound, _np_in_mls_callback_wrapper,
                      np_evt_callback_t_sll_compare_type))) {
        ...
        if (IS_AUTHORIZED(intent_token->state)) {
            ...
            if (real_prop->encryption_algorithm == NEUROPIL_ENCRYPTION && strstr(real_prop->msg_subject, "mls_") == NULL && !userdata->result->authorized) {
                pthread_mutex_lock(userdata->result->lock);
                bool *auth = calloc(1, sizeof(bool));
                *auth = true;
                hashtable_set(userdata->result->authorized_clients, intent_token->issuer, auth);
                if(userdata->result->authorized_clients->size >= (userdata->benchmark->num_clients_per_node-1)) {
                    userdata->result->authorized = true;
                }
                pthread_mutex_unlock(userdata->result->lock);
            } else {
                ...
                if (np_mls_is_everyone_authorized(mls_client, real_prop->msg_subject,
                                                  userdata->benchmark->num_clients_per_node) &&
                    !real_prop->mls_is_creator) {
                    np_mls_authorize(context, protocol_subject);
                }
            }
        }
    ...
    if (IS_NOT_AUTHORIZED(intent_token->state) && needs_authz == true) {
        ...
        np_util_event_t authz_event = {.type=(evt_token | evt_external |
                                              evt_authz), .context=context, .user_data=intent_token, .target_dhkey=authz_target};
